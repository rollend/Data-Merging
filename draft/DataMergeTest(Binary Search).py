...
class JSON_Objects(object):    
            ...
            while True:                      
                try:
                    for line in f:                        
                        ...
                        self.update_progress(round(ln/self.length,3)) 
                        print("Mergeing Successed Lines:{}".format(ln))
                                                                                     
                        if self.seen:
                            add = self.linearSearch(d)                            
                            """
                            add2 = self.binarySearch(list(filtered_list), 0, len(sorted_list)-1, d)
                            """
                            if add:
                                self.seen.append(d)                                                                    
                            else:                                
    ...
    def linearSearch(self, d):        
        """
        filtered_list = filter(lambda d: (d["title"] in d["title"], d["city"] in d["city"], d["country"] in d["country"]), self.seen)
        """
    ...
    def binarySearch(self, arr, l, r, x):
        ...
        if r >= l:     
            ...
            if self.variableCheck(x,dd) and int(x['score']) > int(dd['score']):
                self.seen[idx].update(d)
                return False
    ...
    def dump(self):             
        with open("mock.json", "w") as file:
            ...
            for i in self.gen:
                ...
                ln = ln + 1                                
                self.update_progress(round(ln/len(self.seen),3))                
                print("Dumping Successed Lines:{}".format(ln))
